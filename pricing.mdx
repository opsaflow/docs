...
Pricing is set **per region** (USD/month). For HA tiers (Starter, Growth, Scale) the price is **per node**; Solo is a single node. **EU Central is the cheapest region**, US East is the most expensive, and Singapore sits in between.

| Plan    | EU Central | US East | Asia Pacific (Singapore) |
|---------|------------|---------|--------------------------|
| Solo    | $15        | $29     | $25                      |
| Starter | $35/node   | $59/node | $45/node                |
| Growth  | $59/node   | $119/node | $85/node               |
| Scale   | $99/node   | $229/node | $169/node              |
...
Prices shown are for EU Central, the cheapest region. US East and Singapore are higher — see [Per-region pricing](#per-region-pricing). EU runs on Hetzner's Intel CX line; US East and Singapore run on AMD CPX.

Storage listed is fully available for your PostgreSQL database. An additional 25 GB is reserved for the OS on each node.

### Example pricing

Examples below use **EU Central** rates (the cheapest region). For US East or Singapore, substitute the per-node prices from the [Per-region pricing](#per-region-pricing) table.
...
All regions include the same features. Pricing varies by region — **EU Central is the cheapest** — see [Per-region pricing](#per-region-pricing). Solo hardware specs also vary by region; see the Solo section above.
