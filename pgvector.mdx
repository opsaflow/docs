...
<Info>pgvector is pre-installed on all Rivestack databases. Add it to your database from the dashboard UI.</Info>

## Enable the extension

Add pgvector to your database from the dashboard, then verify it's active:
...
Rivestack pgvector performance on a **Growth** server (4 vCPU, 8 GB RAM) with 1M vectors (1536 dimensions, HNSW index):

| Metric | Result |
|--------|--------|
| **Queries per second** | 2,000 QPS |
| **p50 latency** | <4ms |
| **p99 latency** | <12ms |
| **Recall @ ef_search=100** | 99.1% |

<Info>Benchmarks ran with 10 concurrent clients performing cosine similarity searches against 1M OpenAI ada-002 embeddings using HNSW (`m=16`, `ef_construction=200`).</Info>
