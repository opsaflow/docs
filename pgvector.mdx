...
<Info>QPS and p50 latency trade off across the concurrency curve — they are not simultaneous best-cases, so each figure carries its recall and client count. recall@10 0.93 uses `ef_search=80`; recall@10 0.75 uses `ef_search=10`. Pooling and TLS add only ~0.4 ms over the raw same-region network floor. A 1M × 1536 index is ~6 GB and does not fit or build on a 4 GB node — use a Growth or Scale node for datasets that large. Full methodology and reproduce-it steps: [pgvector NVMe vs cloud SSD benchmarks](https://rivestack.io/blog/pgvector-performance-nvme-vs-cloud-ssd-benchmarks).</Info>
